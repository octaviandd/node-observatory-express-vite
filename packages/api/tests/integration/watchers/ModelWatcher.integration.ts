  ...
  describe("index endpoint", () => {
    ...
    it("should return model entries", async () => {
      await database.insert([
        createModelEntry(
          "model:1",
          { modelName: "User", method: "findAll" },
          { duration: 100, request_id: "req-1" },
        ),
        createModelEntry(
          "model:2",
          { modelName: "Post", method: "create" },
          { duration: 50, request_id: "req-2" },
        ),
      ]);
    ...
    it("should group by model name", async () => {
      await database.insert([
        ...
        createModelEntry(
          "model:2",
          { modelName: "User", method: "findById" },
          { duration: 50 },
        ),
      ]);
    ...
    it("should filter by status", async () => {
      await database.insert([
        ...
        createModelEntry(
          "model:failed",
          { modelName: "User", method: "create" },
          {
            status: "failed",
            error: { name: "ValidationError", message: "Invalid data" },
          },
        ),
      ]);
