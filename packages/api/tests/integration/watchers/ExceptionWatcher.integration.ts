...
import ExceptionWatcher from '../../../src/core/watchers/ExceptionWatcher';
...
describe('ExceptionWatcher Integration', () => {
  ...
  const createExceptionEntry = (
    ...
    content: {
      ...
      data: {
        ...
        stack: data.stack || `Error: ${data.message}\n    at ${data.file || 'test.js'}:${data.line || '1'}:1`,
  ...
  beforeEach(async () => {
    await resetAll();
    watcher = new ExceptionWatcher(redisClient as any, database);
  });
  ...
  describe('index endpoint', () => {
    ...
    it('should return exception entries', async () => {
      await database.insert([
        createExceptionEntry('exception:1', { type: 'uncaughtException', message: 'Test error 1', file: 'test.js', line: '1' }),
        createExceptionEntry('exception:2', { type: 'unhandledRejection', message: 'Test error 2', file: 'test.js', line: '2' }),
      ]);
    ...
    it('should group exceptions by error message', async () => {
      await database.insert([
        createExceptionEntry('exception:1', { type: 'uncaughtException', message: 'Same error' }),
        createExceptionEntry('exception:2', { type: 'uncaughtException', message: 'Same error' }),
      ]);
    ...
    it('should filter by exception type', async () => {
      await database.insert([
        createExceptionEntry('exception:uncaught', { type: 'uncaughtException', message: 'Uncaught error' }),
        createExceptionEntry('exception:unhandled', { type: 'unhandledRejection', message: 'Unhandled rejection' }),
      ]);

      const req = createMockRequest({ table: 'true', index: 'instance', period: '24h', status: 'uncaughtException' });
      const result = await watcher.index(req);
  ...
  describe('view endpoint', () => {
    it('should return entry data by uuid', async () => {
      await database.insert([
        createExceptionEntry('exception:view-test', { type: 'uncaughtException', message: 'Test error' }),
      ]);
    ...
    it('should include related request when request_id exists', async () => {
      ...
      await database.insert([
        createExceptionEntry('exception:related', { type: 'uncaughtException', message: 'Error in request', file: 'handler.js', line: '10' }, { request_id: requestId }),
        {
          uuid: 'request:source',
          type: 'request',
          content: {
            ...
            data: { route: '/api/users', statusCode: 500, requestSize: 0, responseSize: 0 },
    ...
    it('should include related job when job_id exists', async () => {
      ...
      await database.insert([
        createExceptionEntry('exception:job-related', { type: 'uncaughtException', message: 'Error in job' }, { job_id: jobId }),
        {
          uuid: 'job:source',
          type: 'job',
          content: {
            status: 'failed',
            duration: 100,
            metadata: { package: 'bull', method: 'processJob', queue: 'email', connectionName: 'localhost:6379' },
