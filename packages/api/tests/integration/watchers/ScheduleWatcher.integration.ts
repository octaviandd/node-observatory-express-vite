...
import ScheduleWatcher from '../../../src/core/watchers/ScheduleWatcher';
...
describe('ScheduleWatcher Integration', () => {
  ...
  beforeEach(async () => {
    await resetAll();
    watcher = new ScheduleWatcher(redisClient as any, database);
  });
  ...
  describe('index endpoint', () => {
    ...
    it('should return schedule entries', async () => {
      await database.insert([
        createScheduleEntry('schedule:1', { scheduleId: 'daily-backup', cronExpression: '0 0 * * *', type: 'processJob' }, { duration: 5000, schedule_id: 'daily-backup' }),
        createScheduleEntry('schedule:2', { scheduleId: 'hourly-cleanup', cronExpression: '0 * * * *', type: 'processJob' }, { duration: 1000, schedule_id: 'hourly-cleanup' }),
      ]);
    ...
    it('should group by schedule ID', async () => {
      await database.insert([
        createScheduleEntry('schedule:1', { scheduleId: 'daily-backup', cronExpression: '0 0 * * *', type: 'processJob' }, { duration: 5000 }),
        createScheduleEntry('schedule:2', { scheduleId: 'daily-backup', cronExpression: '0 0 * * *', type: 'processJob' }, { duration: 4500 }),
      ]);
    ...
    it('should filter by status', async () => {
      await database.insert([
        createScheduleEntry('schedule:completed', { scheduleId: 'task-1', cronExpression: '* * * * *', type: 'processJob' }, { status: 'completed' }),
        createScheduleEntry('schedule:failed', { scheduleId: 'task-2', cronExpression: '* * * * *', type: 'processJob' }, { status: 'failed', error: { name: 'Error', message: 'Task failed' } }),
      ]);
  ...
  describe('view endpoint', () => {
    it('should return entry data by uuid', async () => {
      await database.insert([
        createScheduleEntry('schedule:view-test', { scheduleId: 'daily-backup', cronExpression: '0 0 * * *', type: 'processJob' }, { schedule_id: 'daily-backup' }),
      ]);
    ...
    it('should include related entries', async () => {
      ...
      await database.insert([
        createScheduleEntry('schedule:main', { scheduleId, cronExpression: '0 0 * * *', type: 'processJob' }, { schedule_id: scheduleId }),
        {
          uuid: 'log:related',
          type: 'log',
          content: {
  ...
  describe('insertRedisStream', () => {
    it('should add entry to Redis stream', async () => {
      const entry = {
        ...
        data: { scheduleId: 'daily-backup', cronExpression: '0 0 * * *', type: 'processJob' },
        location: { file: 'scheduler.ts', line: '30' },
      };
    ...
    it('should handle failed schedules with error', async () => {
      const entry = {
        ...
        duration: 100,
        metadata: { package: 'node-cron' as const, type: 'processJob' },
        data: { scheduleId: 'failing-task', cronExpression: '* * * * *', type: 'processJob' },
        location: { file: 'scheduler.ts', line: '30' },
