...
import NotificationWatcher from '../../../src/core/watchers/NotificationWatcher';
...
describe('NotificationWatcher Integration', () => {
  ...
  beforeEach(async () => {
    await resetAll();
    watcher = new NotificationWatcher(redisClient as any, database);
  });
  ...
  describe('index endpoint', () => {
    ...
    it('should return notification entries', async () => {
      await database.insert([
        createNotificationEntry('notification:1', { channel: 'private-user-1', event: 'message', payload: { text: 'Hello' } }, { duration: 50 }),
        createNotificationEntry('notification:2', { channel: 'presence-room-1', event: 'join', payload: { userId: '123' } }, { duration: 30 }),
      ]);
    ...
    it('should group by channel', async () => {
      await database.insert([
        createNotificationEntry('notification:1', { channel: 'private-user-1', event: 'message' }, { duration: 50 }),
        createNotificationEntry('notification:2', { channel: 'private-user-1', event: 'typing' }, { duration: 30 }),
      ]);
    ...
    it('should filter by status', async () => {
      await database.insert([
        createNotificationEntry('notification:completed', { channel: 'private-user-1', event: 'message' }, { status: 'completed' }),
        createNotificationEntry('notification:failed', { channel: 'private-user-2', event: 'message' }, { status: 'failed', error: { name: 'AblyError', message: 'Connection lost' } }),
      ]);
  ...
  describe('view endpoint', () => {
    it('should return entry data by uuid', async () => {
      await database.insert([
        createNotificationEntry('notification:view-test', { channel: 'private-user-1', event: 'message', payload: { text: 'Test' } }),
      ]);
    ...
    it('should include related request', async () => {
      ...
      await database.insert([
        createNotificationEntry('notification:related', { channel: 'private-user-1', event: 'message' }, { request_id: requestId }),
        {
          uuid: 'request:source',
          type: 'request',
          content: {
            ...
            data: { route: '/api/notify', statusCode: 200, requestSize: 0, responseSize: 50 },
  ...
  describe('insertRedisStream', () => {
    it('should add entry to Redis stream', async () => {
      const entry = {
        ...
        duration: 50,
        metadata: { package: 'ably' as const, method: 'publish', mode: 'realtime' as const },
        data: { channel: 'private-user-1', event: 'message', payload: { text: 'Hello' } },
        location: { file: 'notifications.ts', line: '20' },
      };

      await watcher.insertRedisStream(entry as any);

      const streamLen = await redisClient.xLen('observatory:stream:notification');
    ...
    it('should handle different methods', async () => {
      ...
      for (const method of methods) {
        const entry = {
          status: 'completed' as const,
          duration: 50,
          metadata: { package: 'ably' as const, method, mode: 'realtime' as const },
