...
describe("CacheWatcher Integration", () => {
  ...
  const createCacheEntry = (
    ...
    content: {
      ...
      metadata: {
        package: "redis" as const,
        command: options.command || "get",
      },
      data,
  ...
  describe("index endpoint", () => {
    ...
    it("should return instance data when entries exist", async () => {
      await database.insert([
        createCacheEntry(
          "cache:1",
          { key: "user:1", hits: 10, misses: 2, writes: 5 },
          { duration: 50 },
        ),
        createCacheEntry(
          "cache:2",
          { key: "user:2", hits: 5, misses: 1, writes: 3 },
          { duration: 30 },
        ),
      ]);
    ...
    it("should return grouped data by cache key", async () => {
      await database.insert([
        createCacheEntry(
          "cache:1",
          { key: "user:1", hits: 1, misses: 0, writes: 0 },
          { duration: 10 },
        ),
        createCacheEntry(
          "cache:2",
    ...
    it("should filter by cache type", async () => {
      await database.insert([
        createCacheEntry(
          "cache:hit",
          { key: "test", hits: 1, misses: 0, writes: 0 },
          { duration: 10 },
        ),
      ]);
