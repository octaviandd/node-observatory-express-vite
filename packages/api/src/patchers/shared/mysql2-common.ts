...
type Mysql2Metadata = { package: "mysql2"; context: string; sqlType: string };
type Mysql2Data = { sql: string; hostname: string; port: string | number; database: string };

export type Mysql2QueryLogEntry = BaseLogEntry<Mysql2Metadata, Mysql2Data>;

const timestamp = () => new Date().toISOString().replace("T", " ").substring(0, 19);

function log(entry: Mysql2QueryLogEntry) { 
...
function getConnectionConfig(ctx: any) {
  return {
    hostname: ctx.config?.host || ctx.pool?.config?.connectionConfig?.host || "unknown",
    port: ctx.config?.port || ctx.pool?.config?.connectionConfig?.port || "unknown",
    database: ctx.config?.database || ctx.pool?.config?.connectionConfig?.database || "unknown",
    user: ctx.config?.user || ctx.pool?.config?.connectionConfig?.user || "unknown",
  };
}

function patchMethod(target: any, method: "query" | "execute", contextName: string, filename: string) {
  ...
  shimmer.wrap(target, method, (original: Function) =>
    async function patched(this: any, sqlOrOptions: any, values?: any) {
      ...
      const base = {
        metadata: { package: "mysql2" as const, context: contextName, sqlType: getSqlType(sql) },
        data: { sql, ...getConnectionConfig(this) },
      };
      ...
      try {
        ...
        log({ status: "failed", duration: parseFloat((performance.now() - startTime).toFixed(2)), error: { name: "Mysql2Error", message: error?.message, stack: error?.stack }, ...base });
        throw error;
      }
    }
  );
...
export function patchMysql2Exports(exports: any, filename: string): any {
  shimmer.wrap(exports, "createConnection", (originalCreateConnection: Function) =>
