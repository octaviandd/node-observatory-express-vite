...
if (
  ...
  if (!patchedGlobal[PATCHERS_GLOBAL_SYMBOLSPATCHERS_GLOBAL_SYMBOLS.BREE_PATCHED_SYMBOL]) {
    ...
    addHook((exports: any, name: Namespace, baseDir?: string) => {
      ...
      const BreeExport = exports.default || exports;

      // Patch Bree constructor to intercept job creation
      shimmer.wrap(BreeExport, 'default', function (originalConstructor: any) {
      ...
      const BreePrototype = BreeExport.prototype || BreeExport.default?.prototype;

      if (BreePrototype) {
        // Patch instance methods
        instanceMethods.forEach((method) => {
          if (typeof BreePrototype[method] === 'function') {
            shimmer.wrap(BreePrototype, method, function (originalFn) {
        ...
        if (BreePrototype.init) {
          const originalInit = BreePrototype.init;
          BreePrototype.init = function patchedInit(
            this: any,
            ...args: any[]
          ) {
            const callerInfo = getCallerInfo(__filename);
