...
class MailWatcherSQL extends BaseBuilder {
   ...
   */
  public getIndexTableDataByInstanceSQL(filters: MailFilters) {
    ...
    const mailToSql = key
      ? `AND JSON_CONTAINS(JSON_EXTRACT(content, '$.data.to'), '"${key}"')`
      : "";
   ...
   */
  public getIndexTableDataByGroupSQL(filters: MailFilters) {
   ...
   */
  public getIndexGraphDataSQL(filters: MailFilters) {
