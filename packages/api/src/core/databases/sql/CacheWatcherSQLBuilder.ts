...
class CacheWatcherSQL extends BaseBuilder {
  ...
  public getIndexGraphDataSQL(filters: CacheFilters) {
    ...
    const aggregateColumns = [
      ...
      "CAST(COALESCE(MIN(CAST(JSON_UNQUOTE(JSON_EXTRACT(content, '$.metadata.duration')) AS DECIMAL(10,2))), 0) AS DECIMAL(10,2)) as shortest",
      "CAST(COALESCE(MAX(CAST(JSON_UNQUOTE(JSON_EXTRACT(content, '$.metadata.duration')) AS DECIMAL(10,2))), 0) AS DECIMAL(10,2)) as longest",
      "CAST(COALESCE(AVG(CAST(JSON_UNQUOTE(JSON_EXTRACT(content, '$.metadata.duration')) AS DECIMAL(10,2))), 0) AS DECIMAL(10,2)) as average",
      this.getP95SQL("cache"),
      "NULL as created_at",
      "NULL as content",
      "'aggregate' as type",
