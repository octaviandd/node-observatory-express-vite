...
class Base {
  ...
  async getByGroup<T extends WatcherType>(
    ...
    try {
      ...
      const [results] = (await this.storeConnection.query(sql.items)) as [
        QueryResult,
        FieldPacket[],
      ];
      const [count] = (await this.storeConnection.query(sql.count)) as [
        any[],
        FieldPacket[],
      ];
      return {
  ...
  async getCountGraphData<T extends WatcherType>(
    ...
    try {
      ...
      const sql = this.builders[watcherType].getIndexGraphDataSQL(
        filters as any,
      );

      const [results] = (await this.storeConnection.query(sql)) as [
        QueryResult,
        FieldPacket[],
      ];
