...
class LogWatcherSQL extends BaseBuilder {
  ...
  private getLogTypeSQL(logType: string): string {
    ...
    const conditions = types.map(
      (type) =>
        `JSON_EXTRACT(content, '$.metadata.level') LIKE '%${type.toLowerCase()}%'`,
    );
   ...
   */
  public getIndexTableDataByInstanceSQL(filters: LogFilters) {
    const { limit, offset, status, query, key, period } = filters;

    const typeSql = this.getLogTypeSQL(status);
    const querySql = query ? this.getInclusionSQL(query, "message") : "";
    const messageSql = key
      ? `AND JSON_UNQUOTE(JSON_EXTRACT(content, '$.data.message')) = '${key}'`
      : "";
   ...
   */
  public getIndexTableDataByGroupSQL(filters: LogFilters) {
    ...
    const levelSumColumns = levels.map(
      (lvl) =>
        `SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(content, '$.metadata.level')) LIKE '${lvl}' THEN 1 ELSE 0 END) as ${lvl}`,
    );
   ...
   */
  public getIndexGraphDataSQL(filters: LogFilters) {
