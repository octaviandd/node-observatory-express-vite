...
class LogWatcherSQL extends BaseBuilder {
	...
	private getLogTypeSQL(logType: string): string {
		...
		const conditions = types.map((type) => `JSON_EXTRACT(content, '$.metadata.level') LIKE '%${type.toLowerCase()}%'`);
	...
	public getIndexTableDataByGroupSQL(filters: LogFilters) {
		...
		const columns = ["JSON_UNQUOTE(JSON_EXTRACT(content, '$.data.message')) as message", "COUNT(*) as total", ...levelSumColumns];
	...
	public getIndexGraphDataSQL(filters: LogFilters) {
		...
		const rowColumns = ["NULL as total", ...levels.map((lvl) => `NULL as ${lvl}`), "created_at", "content", "'row' as type"];
