...
class HTTPClientWatcherSQL extends BaseBuilder {
  ...
  public getIndexGraphDataSQL(filters: HTTPClientFilters) {
    ...
    const aggregateColumns = [
      "COUNT(*) as total",
      "MIN(CAST(JSON_EXTRACT(content, '$.metadata.duration') AS DECIMAL)) as shortest",
      "MAX(CAST(JSON_EXTRACT(content, '$.metadata.duration') AS DECIMAL)) as longest",
      "AVG(CAST(JSON_EXTRACT(content, '$.metadata.duration') AS DECIMAL)) as average",
