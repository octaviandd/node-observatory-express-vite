...
class GenericWatcher extends BaseWatcher {
  ...
  constructor(redisClient: RedisClientType, DBInstance: Database, config: WatcherConfig) {
    super(redisClient, DBInstance, config.type as WatcherType);
    this.config = config;
  }

  protected async getTableData(filters: any): Promise<{ results: any, count: string}> {
    if (filters.index === 'instance') {
      const { results, count } = await this.DBInstance.getByInstance(filters, this.type);
      return { results, count: formatValue(count, true) };
    } else {
      const { results, count } = await this.DBInstance.getByGroup(filters, this.type);
      return { results, count: formatValue(count, true) };
    }
  }
  ...
  protected async getGraphData(filters: any): Promise<any> {
    return await this.DBInstance.getGraphData(filters, this.type, this.config.graphMetrics as string[]);
  }
