...
export const Details = ({
  ...
  const stats = {
    ...
    queryAverage: averageOf(queryItems, (item) =>
      Number(item.content.metadata.duration),
    ),
    queryWrites: sumOf(queryItems, (item) =>
      item.content.data.sqlType === "WRITE" ||
      item.content.data.sqlType === "INSERT" ||
      item.content.data.sqlType === "UPDATE" ||
      item.content.data.sqlType === "DELETE"
        ? 1
        : 0,
    ),
    queryReads: sumOf(queryItems, (item) =>
      item.content.data.sqlType === "SELECT" ||
      item.content.data.sqlType === "SHOW"
        ? 1
        : 0,
    ),
