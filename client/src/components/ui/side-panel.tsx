...
import { CacheInstanceResponse, ExceptionInstanceResponse, HttpClientInstanceResponse, JobInstanceResponse, LogInstanceResponse, MailInstanceResponse, ModelInstanceResponse, NotificationInstanceResponse, QueryInstanceResponse, RequestInstanceResponse } from "../../../../types";

type Props = {
  setSidePanelData: Dispatch<SetStateAction<{ isOpen: boolean, modelId?: string; requestId?: string; jobId?: string; scheduleId?: string; }>>
  modelId?: string;
  requestId?: string;
  jobId?: string;
  scheduleId?: string;
...
export default function SidePanel({ setSidePanelData, modelId, requestId, jobId, scheduleId, type }: Props) {
  const [logs, setLogs] = useState<LogInstanceResponse[]>([]);
  const [queries, setQueries] = useState<QueryInstanceResponse[]>([]);
  const [notifications, setNotifications] = useState<NotificationInstanceResponse[]>([]);
  ...
  const getRelatedData = async () => {
    ...
    setNotifications(type === 'notifications' && notification ? notification : []);
  ...
  const renderSection = <T extends object> (title: string, count: number, items: T[], CardComponent: React.ComponentType<{item: T}>) => {
  ...
  return (
    <Sheet open={true} onOpenChange={() => setSidePanelData({isOpen: false, modelId: "", requestId: "", scheduleId: "", jobId: ""})}>
        ...
        <SheetHeader className="p-6 flex w-full">
          <div className="flex items-center gap-x-5 justify-between">
            ...
            <Button
              variant="secondary"
              size="icon"
              onClick={() =>  setSidePanelData({isOpen: false, modelId: "", requestId: "", scheduleId: "", jobId: ""})}
            >
              <X className="h-4 w-4" />
            </Button>
          </div>
