        ...
        return async function patchedRequest(
          ...
          if (options?.body) {
            if (
              ...
              options.body.length <= MAX_BODY_SIZE
            ) {
          ...
          try {
            const originalResponse = await originalRequest.call(
              this,
              fullUrl,
              options,
              ...args,
                      ...
                      if (chunks.length > 0) {
                        try {
                          ...
                          if (responseBody.length <= MAX_BODY_SIZE) {
                            loggingObject.responseBody =
                              responseBody.toString("utf-8");
                          }
                        } catch (error) {
                          console.error(
                            "[Patch undici] Error processing response body:",
                            error,
                          );
                        }
                      }
      ...
      if (typeof exports.fetch === "function") {
        shimmer.wrap(exports, "fetch", function (originalFetch) {
          return async function patchedFetch(
            ...
            try {
              const originalResponse = await originalFetch.call(
                this,
                input,
                init,
                ...args,
