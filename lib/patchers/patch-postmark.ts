        ...
        return function PatchedServerClient(this: any, ...args: any[]) {
          ...
          if (client && typeof client.sendEmail === "function") {
            shimmer.wrap(client, "sendEmail", function (originalSendEmail) {
              return async function patchedSendEmail(
                this: any,
                emailData: any,
              ) {
          ...
          if (client && typeof client.sendEmailWithTemplate === "function") {
            shimmer.wrap(
              ...
              function (originalSendWithTemplate) {
                return async function patchedSendEmailWithTemplate(
                  this: any,
                  templateData: any,
                ) {
                  const startTime = performance.now();
                  ...
                  const content = {
                    ...
                    to: Array.isArray(templateData.To)
                      ? templateData.To
                      : [templateData.To],
                    cc: Array.isArray(templateData.Cc)
                      ? templateData.Cc
                      : templateData.Cc
                        ? [templateData.Cc]
                        : [],
                    bcc: Array.isArray(templateData.Bcc)
                      ? templateData.Bcc
                      : templateData.Bcc
                        ? [templateData.Bcc]
                        : [],
                  ...
                  try {
                    const result = await originalSendWithTemplate.call(
                      this,
                      templateData,
                    );
                    const endTime = performance.now();
